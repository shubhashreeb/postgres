...
CREATE OR REPLACE TRIGGER customer_table_changes()
RETURNS TRIGGER AS $$
BEGIN 
  PERFORM pg_notify('customer_table_changes', TG_OP || '|' || NEW.customer_id || '|' || NEW.company_name || '|' NEW.contact_name || '|' NEW.contact_title || '|' NEW.address)
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER customer_table_change_trigger
AFTER INSERT OR UPDATE OR DELETE ON customers
FOR EACH ROW
EXECUTE FUNCTION customer_table_changes();
...
INSERT INTO carts (cart_id, user_id, cart) VALUES ('1', '123',
'{
  "items": [
    {
      "item_id": 111,
